...
const Country = () => {
  ...
  const { countries, countriesLoading, regions, regionsLoading } = useGuest();

  // Select data according to active tab
  const getActiveData = () => {
    if (activeTab === "Local eSIMs")
      return { data: countries, isLoading: countriesLoading };
    if (activeTab === "Regional eSIMs")
      return { data: regions, isLoading: regionsLoading };
  ...
  return (
    <>
      ...
      <ul className="hidden md:flex flex-row items-center mt-10 text-[22px] align-middle">
        {["Local eSIMs", "Regional eSIMs"].map((tab, index) => (
          <li
            key={index}
            onClick={() => {
              setActiveTab(tab);
